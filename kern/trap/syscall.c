...
void sys_env_set_nice(int niceVal)
{
	env_set_nice(curenv,niceVal);
	return;
}
...
static uint32 getHardLimt(void){
	return  curenv->hardLimt;
}
//2017
...
void* sys_sbrk(int increment)
{
    ...
    if(increment==0){
    	return (void*) curenv->segmentBreak;
    }
    else if(increment>0){

    	 uint32 nextBoundry=ROUNDUP(curenv->segmentBreak,PAGE_SIZE) ;
    	 if(nextBoundry==curenv->segmentBreak){
    	 if((curenv->segmentBreak+ROUNDUP(increment,PAGE_SIZE)) > curenv->hardLimt || free_frame_list.size==0){
    			 return (void*)-1;
    	 }

    	 int numberOfPages= ROUNDUP(increment,PAGE_SIZE);
    	 numberOfPages/=PAGE_SIZE;
    	 uint32 breakseg=curenv->segmentBreak;
    	 uint32* page_table ;

    	 for(int i=0; i < numberOfPages ; i++){
    	 if( get_page_table(curenv->env_page_directory,curenv->segmentBreak+3, &page_table)==TABLE_NOT_EXIST){
    		// cprintf("\nfrom the sbrk the page table is not found we will create it \n");
    		 create_page_table(curenv->env_page_directory,curenv->segmentBreak+3);
    	 }
    	 get_page_table(curenv->env_page_directory,curenv->segmentBreak+3, &page_table);
    	  page_table[PTX(curenv->segmentBreak+3)] |= (1 << 10);
    	  curenv->segmentBreak+=PAGE_SIZE;
    	  }
    	// cprintf("\nfrom sbrk the new break is %x \n",curenv->segmentBreak );
    	  return (void*)breakseg;
    	 }
    	 else{
    	    uint32 c= nextBoundry-curenv->segmentBreak;
    	    if(increment>c){
    	     if((curenv->segmentBreak+ROUNDUP(increment,PAGE_SIZE)) > curenv->hardLimt || free_frame_list.size==0){
    	    	   return (void*)-1;
    	    	 }
    	  	 uint32 breakseg= curenv->segmentBreak;
    	     increment-=c;
    	     curenv->segmentBreak+=c;
    	     int numberOfPages= ROUNDUP(increment,PAGE_SIZE);
    	     numberOfPages/=PAGE_SIZE;
    	     uint32* page_table ;
    	     for(int i=0; i < numberOfPages ; i++){
    	    if( get_page_table(curenv->env_page_directory,curenv->segmentBreak, &page_table)==TABLE_NOT_EXIST){
    	    	// cprintf("\nfrom the sbrk the page table is not found we will create it \n");
    	    	 create_page_table(curenv->env_page_directory,curenv->segmentBreak);
    	     }
    	     get_page_table(curenv->env_page_directory,curenv->segmentBreak, &page_table);
    	     page_table[PTX(curenv->segmentBreak)] |= (1 << 10);
    	     curenv->segmentBreak+=PAGE_SIZE;
    	    }
    	   	 return (void*)breakseg;
    	      }
    	...
    	// cprintf("\n\n\nsbrk decrement case has been called\n\n\n");

    	 increment*=-1;
    	 uint32 nextBoundry=ROUNDDOWN(curenv->segmentBreak,PAGE_SIZE)  ;
    	  if(nextBoundry==curenv->segmentBreak){//case 1 the brk  is aligned
    	...
    	if( get_page_table(curenv->env_page_directory,curenv->segmentBreak, &page_table)==TABLE_NOT_EXIST){
    		...
    		get_page_table(curenv->env_page_directory,curenv->segmentBreak, &page_table);
    		page_table[PTX(curenv->segmentBreak)] &= ~(1 << 10);
    		int presentBit = (page_table[PTX(curenv->segmentBreak)] ) & 1;
    		if(presentBit==1){
    		 uint32 wsSize = LIST_SIZE(&(curenv->page_WS_list));
    		 if(wsSize >= (curenv->page_WS_max_size)){
    		    curenv->page_last_WS_element=NULL;
    		 }
    		 env_page_ws_invalidate(curenv,curenv->segmentBreak);
    		 unmap_frame(curenv->env_page_directory,curenv->segmentBreak);
    		 pf_remove_env_page(curenv,curenv->segmentBreak);
    		}
    	    curenv->segmentBreak-=PAGE_SIZE;
    	    }
    	    curenv->segmentBreak-=(increment%PAGE_SIZE);
    	    	  }
    	  else{//case 2 the brk  is not aligned
    	     uint32 c= curenv->segmentBreak-nextBoundry;
    	     if(increment>c){
    	     uint32* page_table ;
    	     if( get_page_table(curenv->env_page_directory,curenv->segmentBreak, &page_table)==TABLE_NOT_EXIST){
    	    	//cprintf("\nfrom the sbrk the page table is not found we will create it \n");
    	    ...
    	    // get_page_table(curenv->env_page_directory,curenv->segmentBreak, &page_table);
    	     ...
    	     env_page_ws_invalidate(curenv,curenv->segmentBreak);
    	     struct WorkingSetElement*it=LIST_FIRST(&curenv->page_WS_list);
    	     while(it!=curenv->page_last_WS_element){
    	     //cprintf("1\n");
    	      LIST_REMOVE(&curenv->page_WS_list,it);
    	      LIST_INSERT_TAIL(&curenv->page_WS_list,it);
