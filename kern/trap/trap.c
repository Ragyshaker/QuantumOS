...
struct Env*last_fualted_env=NULL;
void fault_handler(struct Trapframe *tf)
{
	...
	if (last_fault_va == fault_va&&last_fualted_env==curenv)
	{
		num_repeated_fault++ ;
	...
	else
	{
		...
		if (fault_va >= USTACKTOP && fault_va < USER_TOP)//edited by you
			panic("User: stack underflow exception!%x",fault_va);
	}
	...
	else
	{
		...
		{
			uint32 *page_table;
			/*if(get_page_table(faulted_env->env_page_directory,fault_va,&page_table)==TABLE_NOT_EXIST){
				// cprintf("\nfrom tia the page table is not found we will create it \n");
				 create_page_table(faulted_env->env_page_directory,faulted_env->segmentBreak);
			}*/
			get_page_table(faulted_env->env_page_directory,fault_va,&page_table);
			int readBit = (page_table[PTX(fault_va)] >> 1) & 1;
			int presentBit = (page_table[PTX(fault_va)] ) & 1;
			int markedBit=(page_table[PTX(fault_va)] >> 10)&1;
			if(fault_va>=USER_LIMIT){
		//cprintf("the user is trying to access the kernel the faulted address is :: %x while the user limits is from %x to %x\n\n",fault_va,USTABDATA,USER_LIMIT);
				sched_kill_env(faulted_env->env_id);
			}
			else if(readBit==0&&presentBit==1){
		//  cprintf("the user is accessing a read only page read bit is :: %d\n",readBit);
				sched_kill_env(faulted_env->env_id);
			}
			else if(fault_va>=USER_HEAP_START&&fault_va<USER_HEAP_MAX) {
				if(markedBit==0){
			 // cprintf("\nthe hard limt of the enivroment is %x\n",faulted_env->hardLimt);
				//cprintf("the user is accessing the user heap I will kill him the faulted address is :: %x\n",fault_va);
				  sched_kill_env(faulted_env->env_id);
				}

				//cprintf("the user is accessing the user heap but I will not kill him the faulted address is :: %x\n",fault_va);
			}
			//your code is here
