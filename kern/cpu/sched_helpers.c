...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	//Your code is here
	//Comment the following line
	//cprintf("\nset nice value function has been called \n");
	 e->nice=nice_value;
	if(nice_value>20){
		nice_value=20;
	}
    ...
    if(e->env_status!=ENV_NEW){
    	//cprintf("\nset nice value function has been called and the env status is new\n");
	   fixed_point_t FIXED_PRI_MAX=fix_int(PRI_MAX);
	   fixed_point_t fixed_nice=fix_int(nice_value*2);
	   fixed_point_t temp1 = fix_unscale (e->recentCPU, 4);
	   fixed_point_t temp2 = fix_sub(FIXED_PRI_MAX,temp1);
	   fixed_point_t temp3 =fix_sub(temp2 ,fixed_nice);
	   e->priority =fix_trunc(temp3);
...
int env_get_recent_cpu(struct Env* e)
{
	...
	//Your code is here
	//Comment the following line
	fixed_point_t temp =fix_scale (e->recentCPU, 100);
...
int get_load_average()
{
	...
	//Your code is here
	//Comment the following line
	fixed_point_t temp =fix_scale (loadAVG, 100);
	return  fix_round(temp);
}
/********* for BSD Priority Scheduler *************/
//==================================================================================//
