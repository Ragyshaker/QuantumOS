	...
	*allocated_pages = 0;
	...
	for (; iVA < end_vaddr && i<remaining_ws_pages; i++, iVA += PAGE_SIZE)
	{
		...
		if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX))
		{
			...
			//Remove wse from page_WS_list
			LIST_REMOVE(&(e->page_WS_list), wse);
			//Always leave 1 page in Active list for the stack because it will be created at the end of env_create()function
...
void initialize_environment(struct Env* e, uint32* ptr_user_page_directory, unsigned int phys_user_page_directory)
{
	...
	e->nice=0;                    //edited by me
	e->recentCPU=fix_int(0);      //edited by me
	e->priority=PRI_MAX;          //edited by me
	int i;
