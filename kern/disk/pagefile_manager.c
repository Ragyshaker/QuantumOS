...
int read_disk_page(uint32 dfn, void* va)
{
	...
	//cprintf("reading from disk to mem addr %x at sector %d\n",va,df_start_sector);
	//LOG_STATMENT( cprintf("reading from disk to mem addr %x at sector %d\n",va,df_start_sector);  );
	int success = ide_read(df_start_sector, (void*)va, SECTOR_PER_PAGE);
	//LOG_STATMENT( if(success==0) {cprintf("read from disk successuflly.\n");} else {cprintf("read from disk failed !!\n");} );
	//cprintf("   success is %d\n",success);
	return success;
}
...
int get_disk_page_table(uint32 *ptr_disk_page_directory, const uint32 virtual_address, int create, uint32 **ptr_disk_page_table)
{
	...
	if(USE_KHEAP && !CHECK_IF_KERNEL_ADDRESS(virtual_address))
	{
		*ptr_disk_page_table = (uint32*)kheap_virtual_address(EXTRACT_ADDRESS(disk_page_directory_entry));
		//cprintf("\nfrom get disk page table case 1 %d-- %x-------%x\n",create,kheap_virtual_address(EXTRACT_ADDRESS(disk_page_directory_entry)),virtual_address);
	}
	else
	{
		*ptr_disk_page_table = STATIC_KERNEL_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(disk_page_directory_entry)) ;
		//cprintf("\nfrom get disk page table case 2\n");
	}
		...
		if (create)
		{
...
#if USE_KHEAP
			{
				*ptr_disk_page_table = (uint32*)kmalloc(PAGE_SIZE);
				...
				{
					//cprintf("\nfrom get disk page table failed to allocate kmalloc returned NULL address\n");
					return E_NO_VM;
				}
				//cprintf("\nfrom get disk page table successed to allocate:: %x\n",kheap_physical_address((unsigned int)*ptr_disk_page_table));
