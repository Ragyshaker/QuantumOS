...
{
//	cprintf("\nÏ am at create working set element\n");
	//uint32 *page_table;
	...
	//uint32 frameNum=(PPN(EXTRACT_ADDRESS(page_table[PTX(virtual_address)])));
	if(newElement==NULL){
		panic("NO_MEM");
	}
	//cprintf("frame number%d \n",frameNum);
    newElement->virtual_address = virtual_address;
   // frames_info[frameNum].element=newElement;
	return newElement;
}
		...
		LIST_FOREACH(wse, &(e->page_WS_list))
		{
			if(ROUNDDOWN(wse->virtual_address,PAGE_SIZE) == ROUNDDOWN(virtual_address,PAGE_SIZE))
			{
				if (e->page_last_WS_element == wse)
				{
					...
					//if(e->page_last_WS_element==NULL)//edited by me
						//e->page_last_WS_element=LIST_FIRST(&(e->page_WS_list));//edited by me
				}
...
void env_page_ws_list_delete_element(struct WorkingSetElement * ptr_WS_element){
	kfree(ptr_WS_element);
}
///=================================================================================================
///=================================================================================================
